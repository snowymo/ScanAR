...
// 			<< "\ntransmat" << transmat << "\n";
	...
	void EstimateAffTransSF(unsigned char * holopoints, unsigned char * vivepoints, unsigned char * transformation, int pointCnt)
	{
		// three points as a set by default
		 		cv::Mat holomat = cv::Mat(pointCnt, 3, CV_64FC1, holopoints);
		 		cv::Mat vivemat = cv::Mat(pointCnt, 3, CV_64FC1, vivepoints);
		...
		//transmat = cv::Mat::ones(3, 4, CV_32FC1);
		memcpy(transformation, transmat.data, sizeof transmat.data * 3 * 4);
		//memcpy(holopoints, one43.data, sizeof one43.data * 4 * 3);
		//memcpy(vivepoints, one43.data, sizeof one43.data * 4 * 3);
		//transformation = transmat.data;
	}
